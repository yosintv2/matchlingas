export default function SocialShare({ match }) {
  ...
  const text = encodeURIComponent(
    `${match?.team1 || ''} vs ${match?.team2 || ''} - ${match?.league || ''} | YoSinTV`
  );
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      <span className="text-xs font-semibold text-gray-500">Share This Match:</span>
        {shareLinks.map(link => (
        <a
          key={link.name}
          href={link.href}
          ...
          className={`w-8 h-8 rounded-full ${link.bg} flex items-center justify-center transition-all hover:scale-110`}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="white">{link.svg}</svg>
        </a>
        ))}
    </div>
  );
